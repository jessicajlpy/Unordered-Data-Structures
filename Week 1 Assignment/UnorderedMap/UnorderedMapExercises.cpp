...
StringIntMap makeWordCounts(const StringVec& words) { //reference to stringvector words
  StringIntMap wordcount_map; //cause stringintmap has originally been defined to map key to frequency, using StringIntMap = std::unordered_map<std::string, int>
  for (auto item: words) { //iterate through each item in the vector 
    if (wordcount_map.find(item) == wordcount_map.end()) { //meaning item does not exist
        wordcount_map[item] = 1; //insert into map 
    }

    else {
        wordcount_map[item]++; //the frequency increases 
    }
  }
  // =================================================
...
int lookupWithFallback(const StringIntMap& wordcount_map, const std::string& key, int fallbackVal) {
  for (auto iterate = wordcount_map.find(key); iterate != wordcount_map.end(); iterate++) { //traverse map for key 
    return iterate->second; //meaning return the frequency 
  }
